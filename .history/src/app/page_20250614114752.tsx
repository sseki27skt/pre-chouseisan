...
export default function MultiTimeScheduler() {
  ...
  // ▼▼▼ ロジックを修正: スライダーの範囲に応じて15分枠をすべて選択するシンプルなロジックに変更 ▼▼▼
  useEffect(() => {
    // このuseEffectは時間選択モードの時だけ動作する
    ...
    const newSelectedTimes = allTimes.filter(time => {
      ...
      // 時間がスライダーの範囲内にあるかだけをチェック
      return timeValue >= startHourFloat && timeValue <= endHourFloat;
    });
    setSelectedTimes(newSelectedTimes);
  }, [timeRange, selectionMode]);
  ...
  const handleClearAll = () => {
    setSelectedDates([]);
    setTimeRange([9, 17]);
    setSelectedTimes([]); // useEffectで再計算されるが、明示的にクリア
    setSelectedPeriods([]);
    setOutput("");
  };
            ...
            <TabsContent value="time" forceMount className="mt-4">
              <div className="p-4 border rounded-lg mb-6">
                 <div className="flex justify-between items-center mb-2">
                    <Label htmlFor="time-range" className="font-medium">時間範囲</Label>
