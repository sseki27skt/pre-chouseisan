...
export default function MultiTimeScheduler() {
  ...
  // ▼▼▼ 3. 選択モードと、時間・時限選択の状態を管理するstateを追加
  ...
  useEffect(() => {
    if (selectionMode !== 'time') return; // 時間選択モードの時のみ実行
  ...
  // ▼▼▼ 5. handleGenerateを更新し、選択モードに応じて出力を切り替える
    ...
    if (selectionMode === 'time') {
      ...
      combinations = selectedDates.flatMap((date) =>
        periodsToGenerate.map((period) => {
          const formattedDate = format(date, "yyyy-MM-dd");
          return {
            datetime: new Date(`${formattedDate}T${period.startTime}`), // ソート用に開始時刻を使用
            display: `${dateLabel(date)} ${period.name}`, // 表示は時限名
          };
        })
      );
    }
  ...
  const handleCopyAndGo = () => { /* ... 変更なし ... */ };
  const handleCopyOnly = () => { /* ... 変更なし ... */ };
    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      <Card className="mb-4">
        <CardContent className="p-4">
          ...
          {/* ▼▼▼ 6. TabsコンポーネントでUIを切り替える ▼▼▼ */}
          <Tabs value={selectionMode} onValueChange={(value) => setSelectionMode(value as 'time' | 'period')}>
            ...
            <TabsContent value="time" className="mt-4">
              ...
              <div className="p-4 border rounded-lg mb-6 space-y-6">
                <div>
                  ...
                  <Slider id="time-range" value={timeRange} onValueChange={(value) => setTimeRange([value[0], value[1]])} max={24} min={0} step={0.5} className="my-4"/>
                </div>
                <div>
                  ...
                  <RadioGroup value={String(timeStep)} onValueChange={(value) => setTimeStep(Number(value))} className="flex flex-wrap items-center gap-x-6 gap-y-2">
                    <RadioGroupItem value="180" id="r5" /><Label htmlFor="r5">3時間</Label>
                    <RadioGroupItem value="120" id="r4" /><Label htmlFor="r4">2時間</Label>
                    <RadioGroupItem value="60" id="r1" /><Label htmlFor="r1">60分</Label>
                    <RadioGroupItem value="30" id="r2" /><Label htmlFor="r2">30分</Label>
                    <RadioGroupItem value="15" id="r3" /><Label htmlFor="r3">15分</Label>
                  </RadioGroup>
                </div>
              </div>
