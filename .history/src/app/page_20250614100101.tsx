...
// groupedTimesも8:00始まりの順序で
const groupedTimes: { [hour: string]: string[] } = {};
...
const allGroupedTimes = Object.values(groupedTimes).flat();
...
export default function MultiTimeScheduler() {
  ...
  const handleGenerate = () => {
    ...
    const combinations = selectedDates.flatMap((date) =>
      selectedTimes.map((time) => {
        ...
        return { datetime: new Date(`${formattedDate}T${time}`), display: `${dateLabel(date)} ${time}〜` };
      })
    );
  ...
  const handleBulkSelect = (type: 'morning' | 'afternoon' | 'evening' | 'all' | 'none') => {
    ...
    switch (type) {
      case 'morning': newSelectedTimes = allTimes.filter(t => t >= '08:00' && t < '12:00'); break;
 ...
 {/* ★ ここに説明文を追加します */}
      ...
      <Card className="mb-4">
        <CardContent className="p-4 flex flex-col items-center">
          <h2 className="text-xl font-semibold mb-2 self-start">1. 日付を選択</h2>
          <Calendar mode="multiple" selected={selectedDates} onSelect={setSelectedDates} className="rounded-md border mx-auto"/>
        </CardContent>
      </Card>
      <Card className="mb-4">
