...
export default function MultiTimeScheduler() {
  ...
  const handleGenerate = () => {
    ...
    // 時間帯が未選択の場合は「終日」として出力
    const times = selectedTimes.length === 0 ? ["終日"] : selectedTimes;
    const combinations = selectedDates.flatMap((date) =>
      times.map((time) => {
        const formattedDate = format(date, "yyyy-MM-dd");
        return { datetime: new Date(`${formattedDate}T${time === "終日" ? "00:00" : time}`), display: `${format(date, "M/d")} ${time}〜` };
      })
    );
